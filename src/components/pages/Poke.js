...
const Poke = () => {
    ...
    useEffect(() => {
        if(dataFiltrada.length === 0){
            const getPokemons = async (data) => {
                ...
                data?.data?.results.forEach(async(el)=>{
    ...
    }, [data.data, dataFiltrada]);//hooks se ejecutara cada vez que varie data o dataFiltrada
    ...
    const handleSearch = (search) => {
        if(search){
            ...
            setPokemons([]);//para que me limpie los pasados pokemones paginados
            let allFilter = searchFilter(data.data, search);
    ...
    return(
        <section className="container search-cont">
            ...
            <div className="btns-next-prev">
                <button onClick={(e)=>handleLinks(e,data.data.previous)} style={{visibility: data?.data?.previous ? 'visible' : 'hidden' }}>
                ...
                <button onClick={(e)=>handleLinks(e, data.data.next)} style={{visibility: data?.data?.next ? 'visible' : 'hidden' }}>
